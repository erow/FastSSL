...
def init_distributed_mode(args):
    ...
    if torch.cuda.is_available():
        ...
        print('No CUDA or MPS device found')
        args.distributed = False
...
def interpolate_pos_encoding(pos_embed,  w, h, patch_size=16):
    """interpolate pos embedding to w, h

    Args:
        pos_embed (Tensor): pos embedding
        w (int): the width of target image size
        h (int): the height of target image size

    Returns:
        Tensor: pos embedding
    """
...
def load_pretrained_weights(model, weight_path):
    ...
    pos_embed_keys = [k for k in state_dict.keys() if 'pos_embed' in k] # find pos embedding
    for k in pos_embed_keys:
        del state_dict[k] 
    print("Loading pretrained weights from %s" % weight_path)
